...
void PB_Roaming::checkWay( const Vector &targetPos ) 
{
	...
	if (followLeft) {
		...
		if (!checkIfPassage) {  // following the wall
			if (!side.onTouch && (passedEdge<gpGlobals->time)) {  // it has disappeared...
				...
				if ( (fabs(UTIL_AngleDiff(action->moveAngleYaw(),targetAngle.y))<=90) &&
					(fabs(UTIL_AngleDiff(action->moveAngleYaw()+90,targetAngle.y))<=90) ) {
					followLeft=false;
		...
		if (left.blocked && right.blocked) {	// suddenly a wall
			...
			if (left.wallAngle.y==right.wallAngle.y) { // in front of plain wall
				...
				if (fabs(adtw)>135) { // target straight behind
	...
	if (targetVisible) { // special behaviour if target is visible:
		if ( (targetDistance<70) ||	     // - too close to target, ignore walls
			 ((followLeft||followRight) && sideValid && (fabs(UTIL_AngleDiff(side.wallAngle.y,targetAngle.y))<=90)) ) {	
			//if (debugWay) 
